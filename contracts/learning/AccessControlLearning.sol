...
contract AccessControlLearning {
    ...
    mapping(address => mapping(bytes32 => uint256)) private temporarilyRole;
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        (bool hasTempRole, uint256 adminLimit) = hasTemporaryRole(
            msg.sender,
            role
        );
        ...
        if (hasTempRole) {
            adminLimit -= 1;
            temporarilyRole[msg.sender][role] = adminLimit;
            if (adminLimit == 0) {
                _roles[msg.sender][role] = false;
            }
        }
    }
    ...
    function grantRoleTemporarily(
        ...
        require(_limit >= 1, "El limite debe ser mayor que uno");
        uint256 limit = temporarilyRole[_account][_role];
        require(
            limit == 0 && !_roles[_account][_role],
            "Esta cuenta ya tiene un limite asignado"
        );
        temporarilyRole[_account][_role] = _limit;
    ...
    {
        uint256 limit = temporarilyRole[_account][_role];
        return (limit > 0 && _roles[_account][_role], limit);
    }
}
